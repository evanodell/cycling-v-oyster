...
fluidRow(
  ...
  p("You can see in the second plot that since writing the blog post in February 2017 my Oyster spending has dropped off somewhat. Since analysing how much I was cycling, and how much I was spending on transport, I've become much more dedicated to riding places, no longer taking the bus or the tube if I'm feeling a little bit lazy."),
 
# Select Output and fine check --------------------------------------------------------
radioButtons("selection", "Select Travelcard Type",
             choices = c(
               "Weekly" = "week_oyster_per_day",
               "Monthly" = "mon_oyster_per_day",
               "Annual" = "annual_oyster_per_day"),
             selected = "mon_oyster_per_day", inline = TRUE),
...
bike_data$week_oyster_per_day <- case_when(
  bike_data$date <= "2017-01-02" ~ 32.4/7,
  bike_data$date <= "2018-01-02" ~ 33/7,
  bike_data$date <= "2019-01-02" ~ 34.1/7
)

bike_data$mon_oyster_per_day <- case_when(
  bike_data$date <= "2017-01-02" ~ 124.50/30,
  bike_data$date <= "2018-01-02" ~ 126.80/30,
  bike_data$date <= "2019-01-02" ~ 131.00/30
)

bike_data$annual_oyster_per_day <- case_when(
  bike_data$date <= "2017-01-02" ~ 1296/366,
  bike_data$date <= "2018-01-02" ~ 1320/365,
  bike_data$date <= "2019-01-02" ~ 1364/365
)

bike_data$locker_cost <- case_when(
  bike_data$date <= "2018-01-22" ~ 60/365,
  bike_data$date <= "2019-01-22" ~ 30/365,
  bike_data$date <= "2020-01-22" ~ 40/365
)

bike_data$insurance <- case_when(
  bike_data$date >= "2018-06-18" ~ 101.80/365, TRUE ~ 0
)

bike_data$bike <- bike_data$bike + bike_data$locker_cost + 
  bike_data$insurance 

write_rds(bike_data, "bike_data.rds")
...
server <- function(input, output, session) {
  ...
  bike_data <- read_rds("bike_data.rds")
  ...
  output$last_update <- renderText(
    paste0("Last Updated: ", format(max(bike_data$date), 
          format = "%d %B %Y")))
  ...
  output$savings <- renderText({
    ...
    travelcard_total <- sum(bike_data[[input$selection]], bike_data$fines)
  ...
  output$p2 <- renderPlot({
   ...
   7, mean, align = "right")
    ...
    p2 <- ggplot(oyster_roll_gg, aes(x = date)) +
      ...
      geom_text(aes(x = max(date), y = bike_average,
                    hjust= 1, vjust = 1.4,
                    label = paste0("Bike Average: £", 
                                   format(round(bike_average, 2), nsmall = 2))),
                size = 6) +
      geom_text(aes(x = max(date),
                    y = max(bike_data[[input$selection]]),
                    hjust = 1, vjust = -0.5,
                    label = paste0("Travelcard Average: £", format(
                      round(mean(bike_data[[input$selection]]), 2),
                      nsmall = 2))),
                size = 6) +
      geom_step(aes(y = bike_data[[input$selection]],
  ...
  output$p2_text <- renderText({
    ...
    comparison <- sum(bike_data$bike, bike_data$oyster) - 
      sum(bike_data[[input$selection]], bike_data$fines)
  ...
  output$p5 <- renderPlot({
    ...
    bike_data$gain_loss <- cumsum(bike_data[[input$selection]]) -
